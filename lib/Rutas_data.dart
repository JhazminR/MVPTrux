...
class RutasData {
  ...
  // --------------------------------------------------------------------------
  //  2. POLYLINE (¡REEMPLAZA ESTA LISTA CON TUS 357 COORDENADAS!)
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  //  3. DESTINOS CLAVE (Índices: 0%, 33%, 66%, 100%)
  //     Estos son los que aparecerán en la lista de "RECIENTES" del Figma.
  // --------------------------------------------------------------------------
  static final List<Destino> _destinos = [
    Destino(
      nombre: 'Panamericana Norte',
      direccion: 'Au. Panamericana Norte 7660',
      coordenadas: LatLng(-8.0496345, -79.0865509), // coordenadas reales
      puntoIndice: 0, // 0%
    ),
    Destino(
      nombre: 'Parque Amauta - La Esperanza',
      direccion: '6 De Enero 675-733, La Esperanza',
      coordenadas: LatLng(-8.0673121, -79.0569224),
      puntoIndice: 119, // 33%
    ),
    Destino(
      nombre: 'Mall Plaza Trujillo',
      direccion: 'Av. Mansiche s/n',
      coordenadas: LatLng(-8.1009, -79.04927), // coordenadas reales
      puntoIndice: 238, // 66%
    ),
    Destino(
      nombre: 'Av. Del Contador',
      direccion: 'Final de la Av. Del Contador con Prol. Francisco de Zela',
      coordenadas: LatLng(-8.1215942, -79.0100033), //coordenadas reales
      puntoIndice: 356, // 100%
    ),
  ];

  // --------------------------------------------------------------------------
  //  4. GETTER DE LA RUTA PRINCIPAL (Inicializa la polyline si está vacía)
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  //  5. LÓGICA DE CÁLCULO (PROXIMIDAD Y BÚSQUEDA)
  //     Aquí está la magia que usa maps_toolkit o matemática pura.
  // --------------------------------------------------------------------------

  // 5.1 Distancia de un punto a un segmento (en metros)
  //     Fórmula de proyección escalar en el plano (precisa para distancias < 10 km)
  static double _distanciaASegmento(LatLng punto, LatLng p1, LatLng p2) {
    ...
    if (dx == 0 && dy == 0) {
      ...
      return distGrados * 111320; // 1 grado ≈ 111,320 metros
    }

    // Proyección escalar (t) de P en el segmento P1-P2
  ...
  // 5.2 Verificar si el usuario está SOBRE la ruta (tolerancia en metros)
  //     ¡Esto reemplaza al isLocationOnEdge de maps_toolkit!
  ...
  // 5.3 Obtener el punto más cercano de la ruta al usuario (útil para ETA)
  ...
  // 5.4 Obtener el índice más cercano en la polyline (para saber en qué tramo vas)
  ...
  // 5.5 Buscar destino por nombre (para la UI)
  ...
  // 5.6 Obtener todos los destinos (para la lista de sugerencias)
